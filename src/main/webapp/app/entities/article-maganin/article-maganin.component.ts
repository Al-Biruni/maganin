...
export class ArticleMaganinComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.articleService
      ...
      .subscribe(
        (res: HttpResponse<IArticleMaganin[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IArticleMaganin): number {
  ...
  delete(article: IArticleMaganin): void {
  ...
  protected onSuccess(data: IArticleMaganin[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/article-maganin'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
